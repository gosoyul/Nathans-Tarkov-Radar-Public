...
function grub_iommu(){
    ...
    if ! sed -i 's/GRUB_CMDLINE_LINUX=""/GRUB_CMDLINE_LINUX="amd_iommu=on iommu=pt"/g' /etc/default/grub; then
...
function install_libguestfs() {
    ...
    sudo aptitude install -f default-jdk parted libyara3 erlang-dev gperf flex bison libaugeas-dev libhivex-dev supermin ocaml-nox ocaml-findlib libhivex-ocaml genisoimage libhivex-ocaml-dev libmagic-dev libjansson-dev gnulib jq -y 2>/dev/null
    sudo apt update
...
'issues')
    issues;;
'all')
    aptitude install -f language-pack-UTF-8 mlocate
    qemu_func
    seabios_func
    if [ "$OS" = "Linux" ]; then
        install_kvm_linux
